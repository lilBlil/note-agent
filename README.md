...
项目适用于整理论文、技术主题、GitHub 项目、面试知识点和工程方案，也可用于展示 AI Agent 在流程编排、工具调用、质量评估和工程化交付方面的设计能力。
...
| ReAct Tool Calling 动图 | `docs/demo-react.gif` | Agent 推理、工具调用、状态更新 |
...
| Notion 发布 | Markdown -> Notion Blocks 转换器，支持标题、列表、表格、代码块、行内样式和 LaTeX |
...
Fixed Workflow 强调稳定和可控：每个节点职责明确，适合需要固定流程、可观测状态和可复现输出的笔记生成任务。
...
项目包含一个轻量 Evaluation Harness，用于评估 Prompt 与节点逻辑，并跟踪不同迭代设置下的输出质量。
